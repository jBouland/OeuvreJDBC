...
public class slOeuvres extends HttpServlet {
    ...
    private String enregistrerOeuvre(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            p = ProprietaireDAO.getProprio(Integer.parseInt(request.getParameter("lProprietaires")));
            oeuvre.setProprietaire(p);
            oeuvre.setId_proprietaire(p.getId_proprietaire());
            if(id == 0){
                OeuvreDAO.ajouter_Oeuvre(oeuvre);
            }else{
                oeuvre.setId_oeuvre(id);
                OeuvreDAO.update_Oeuvre(oeuvre);
            }
    ...
    private String modifierOeuvre(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            request.setAttribute("oeuvreR", oeu);
            request.setAttribute("lstProprietairesR", ProprietaireDAO.getListProprio());
    ...
    private String supprimerOeuvre(HttpServletRequest request) throws Exception {
        String vueReponse;
        String titre = "placeholder";
        try {
            int id = Integer.parseInt(request.getParameter("id"));
            Oeuvre oeu = OeuvreDAO.getOeuvre(id);
            titre = oeu.getTitre();
            OeuvreDAO.delete_Oeuvre(id);
            vueReponse = "catalogue.oe";
            return (vueReponse);
        } catch (Exception e) {
            erreur = e.getMessage();
    ...
    private String connecter(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            if (ProprietaireDAO.connecter(login, pwd)) {
                Proprietaire proprio = ProprietaireDAO.getProprio(login);
                HttpSession session = request.getSession(true);
                session.setAttribute("userId", proprio.getId_proprietaire());
                request.setAttribute("userR", proprio);
            } else {
    ...
    private String listerOeuvres(HttpServletRequest request) throws Exception {
        ...
        try {
            ArrayList<Oeuvre> lOeuvres;
            lOeuvres = OeuvreDAO.listerOeuvre();
            HttpSession session = request.getSession(true);
